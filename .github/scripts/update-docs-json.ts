...
/**
 * Get the file path prefix based on version and tab name
 */
function getPathPrefix(version: string, tabName: string): string {
  const langPrefix = tabName === 'APIリファレンス' ? 'ja/' : '';
...
/**
 * Get localized group labels based on tab name
 */
function getGroupLabels(tabName: string): { resources: string; endpoints: string } {
  if (tabName === 'APIリファレンス') {
    return {
      resources: 'リソースの一覧',
      endpoints: 'エンドポイント'
    };
...
function generateDocsStructure(
  ...
  const pathPrefix = getPathPrefix(version, tabName);
  const groups: DocsJsonGroup[] = [];
  const labels = getGroupLabels(tabName);
  ...
  for (const language of docs.navigation.languages) {
    ...
    if (tab) {
      ...
      if (groupIndex >= 0) {
        ...
        console.log(`   ✅ Updated existing group at index ${groupIndex}`);
        foundGroup = true;
      } else {
        // Add new group
        tab.groups.push(newGroups[0]);
        console.log(`   ✅ Added new group (total groups: ${tab.groups.length})`);
        foundGroup = true;
      }
      break;
    }
...
function processVersion(
  ...
  const newGroups = generateDocsStructure(allTagGroups, version, indexPage, tabName, locale);

  console.log(`\n📄 Generated ${newGroups.length} group(s) for docs.json`);

  // Update docs.json
  updateDocsJson(docsJsonPath, tabName, version, newGroups);
}
...
async function main(): Promise<number> {
  ...
  let openapiPattern: string | undefined;
  let tabName: string | undefined;
  let docsJsonPath = '../../docs.json';
  let locale = 'en';

  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--openapi' && i + 1 < args.length) {
      openapiPattern = args[++i];
    } else if (args[i] === '--tab' && i + 1 < args.length) {
      tabName = args[++i];
    } else if (args[i] === '--docs-json' && i + 1 < args.length) {
      docsJsonPath = args[++i];
  ...
  if (!openapiPattern || !tabName) {
    console.error('❌ Error: Missing required arguments');
    console.error('');
    console.error('Usage: npm run update-docs-json -- --openapi <pattern> --tab <tab-name> [options]');
    console.error('');
    console.error('Arguments:');
    console.error('  --openapi <pattern>  - Path or glob pattern for OpenAPI JSON file(s)');
    console.error('  --tab <tab-name>     - Tab name: "API reference" or "APIリファレンス"');
    console.error('  --docs-json <path>   - Path to docs.json (default: ../../docs.json)');
    console.error('  --locale <locale>    - Locale for sorting (default: en, e.g., ja, en, fr)');
    console.error('');
    console.error('Examples:');
    console.error('  npm run update-docs-json -- --openapi "../../box-openapi*.json" --tab "API reference"');
    console.error('  npm run update-docs-json -- --openapi "../../box-openapi*.json" --tab "APIリファレンス" --locale ja');
    return 1;
  }

  console.log('🚀 Starting docs.json update...\n');
  console.log(`OpenAPI pattern: ${openapiPattern}`);
  ...
  try {
    ...
    const directory = path.dirname(openapiPattern);
    const pattern = path.basename(openapiPattern);
