...
レスポンスは以下のようになります (説明のため省略されています)。レスポンス全体の例については、<Link href="/reference/get-sign-templates#response-example">Box SignテンプレートAPI</Link>を参照してください。

また、具体的なパラメータの詳細については、<Link href="/reference/post-sign-requests">署名リクエストを作成</Link>ガイドで確認できます。
...
テンプレートを使用して<Link href="/reference/post-sign-requests">署名リクエストを作成</Link>するには、以下の手順に従います。
