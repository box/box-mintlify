...
rank: 1
related_endpoints:
  0: options_files_content
  1: options_files_id_content
  2: post_files_content
  3: post_files_id_content
related_guides:
  0: uploads/direct/file
  1: uploads/direct/file-version
related_pages: {}
related_resources: {}
required_guides: {}
title: 事前チェック
...
新しいファイルのチェックを実行するには、実際のファイルをアップロードする場合と同じパラメータ (バイナリコンテンツを除く) を使用して[`OPTIONS /files/content`](/reference/options-files-content) APIを呼び出してください。
...
新しいファイルバージョンのチェックを実行するには、実際のファイルをアップロードする場合と同じパラメータ (バイナリコンテンツを除く) を使用して[`OPTIONS /files/:id/content`](/reference/options-files-content) APIを呼び出してください。
