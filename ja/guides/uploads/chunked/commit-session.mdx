...
ファイルアップロードセッションをコミットするには、コミットするアップロード済みパーツのリストを指定して<Link href="/reference/post-files-upload-sessions-id-commit">`POST /files/upload_sessions/:id/commit`</Link>を呼び出します。
...
<Info>
  加えて、ファイルの`attributes`を`parts`とともに渡すことで、ファイルに情報を追加できます。詳細については、<Link href="/reference/post-files-content">`POST /files/content`</Link> APIを参照してください。
</Info>

## レスポンス

成功すると、APIはHTTP `201 Created`ステータスコードと<Link href="/reference/resources/file">`File`</Link>オブジェクトを返します。
