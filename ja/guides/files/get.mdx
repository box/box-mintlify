...
ファイルのコンテンツではなく、ファイル自体の情報を取得するには、ファイルの`id`を指定して<Link href="/reference/get-files-id">`GET /files/:id`</Link> APIを呼び出します。
...
<Card href="/guides/api-calls/request-extra-fields" arrow title="追加フィールドのリクエストについて確認する" />
