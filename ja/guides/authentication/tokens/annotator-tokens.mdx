...
related_endpoints:
  0: post_oauth2_token
related_guides:
  0: authentication/jwt/without-sdk
  1: authentication/app-token
related_resources: {}
required_guides:
  0: authentication/app-token
title: 注釈トークン
...
その後、ガイドに従ってこのクレームをアサーションに変換し、このアサーションを、既存の有効なアクセストークン、アプリトークン、またはファイルトークンのほか、スコープのセット、トークンの作成対象となるリソースとともに[`POST /oauth2/token`](/reference/post-oauth2-token)エンドポイントに渡します。
...
<RelatedLinks
  title="関連するガイド"
  items={[
  { label: "JWT without SDKs", href: "/guides/authentication/jwt/without-sdk", badge: "GUIDE" },
  { label: "App Token Auth", href: "/guides/authentication/app-token/index", badge: "GUIDE" }
]}
/>
