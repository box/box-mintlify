...
related_endpoints:
  0: get_authorize
related_guides:
  0: authentication/tokens/downscope
related_resources: {}
required_guides:
  0: authentication/select
  1: authentication/app-token/app-token-setup
...
これを使用すると、アプリケーションは、アプリトークン認証に対して有効になっている[エンドポイント](/guides/authentication/app-token/endpoints)のいずれかにAPIコールを実行できます。
