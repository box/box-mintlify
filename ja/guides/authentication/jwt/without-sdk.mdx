...
このトークンの使用方法を確認するには、[APIコールの実行](/guides/api-calls)に関するガイドを参照してください。
...
<Note>
  ...
  `as-user`ヘッダーを使用するか[ユーザーアクセストークン](/guides/authentication/jwt/user-access-tokens)をリクエストして、[別のユーザーとして処理を実行](/guides/authentication/oauth2/as-user)できます。
</Note>
