...
1. **Box**の**管理コンソール**から \[**統合**] に移動し、**Box MCP Server**を探します。これには、MCPカテゴリフィルタを使用するか、ページ上部にある検索バーを使用します。
2. \[**構成**] に移動します。
3. \[**統合資格情報を追加**] ボタンをクリックして、新しい資格情報を生成します。
4. \[**リダイレクトURI**] で、BoxのリダイレクトURIを次のように変更します: [https://chatgpt.com/connector\_platform\_oauth\_redirect](https://chatgpt.com/connector_platform_oauth_redirect)

**クライアントID**と**クライアントシークレット**が表示されます。これらは後ほど、ChatGPTが承認接続を行うために必要になります。
...
5. 以下の**アクセス**スコープを確認します。
   ...
   * AIリクエストの管理
6. \[**保存**] をクリックします。
7. ChatGPT側では、ChatGPTワークスペースの管理者として、左下にあるプロフィールをクリックします。
8. \[**Settings (設定)**] > \[**Apps & Connectors (アプリとコネクタ)**] > \[**Advanced Settings (詳細設定)**] をクリックします。
9. \[**Developer mode (開発者モード)**] をオンに切り替えます。これにより、カスタムコネクタを追加できます。
10. \[**Apps & Connectors (アプリとコネクタ)**] に戻ります。
11. ウィンドウの右上にある \[**Create (作成)**] をクリックします。
12. データを入力します。
...
13. \[`I understand and want to continue`] チェックボックスをオンにし、\[**Create (作成)**] をクリックします。
14. Boxユーザーを承認します。
...
15. 設定をテストします。そのためには、チャットに移動して、\[**+**] アイコンをクリックした後、\[**More (その他)**] にカーソルを合わせて、新しく追加されたコネクタを選択します。OpenAI Boxコネクタで構築された他のコネクタが無効になっているか切断されていることを確認し、回答がカスタムコネクタから提供されるようにします。
16. このコネクタをワークスペース全体に追加する場合は、[ChatGPTワークスペース](https://help.openai.com/en/articles/12584461-developer-mode-and-full-mcp-connectors-in-chatgpt-beta#:~:text=Publish%20connector,settings%20in%20ChatGPT)内で公開できます。
