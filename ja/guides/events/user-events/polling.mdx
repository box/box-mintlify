---
alias_paths:
  0: /guides/events/polling
  1: /guides/events/polling/
rank: 4
related_endpoints:
  0: get_events
  1: options_events
...
Boxアカウントでアクティビティのリアルタイム通知を取得するために、[`OPTIONS /events`](/reference/options-events) APIのLong polling機能を使用できます。
...
Long pollingを使用するには、まず、リクエストを[`OPTIONS /events`](/reference/options-events) APIに送信し、Long polling URLを取得します。
