...
related_endpoints:
  ...
  1: options_events
related_guides:
  0: events/user-events/for-user
  1: events/user-events/polling
  2: events/parameters/pagination
required_guides: {}
...
Enterprise Eventを取得するには、`stream_type`を`admin_logs`または`admin_logs_streaming`に設定して[`GET /events`](/reference/get-events) APIを呼び出します。
