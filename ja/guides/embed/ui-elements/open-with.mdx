...
related_resources: {}
required_guides:
  0: embed/ui-elements/installation
...
現在、この要素では、認証用に[App User](/platform/user-types)のみがサポートされています。

## インストール

NPMまたはBox CDN経由でBox UI Elementsをインストールする方法は、[こちら](/guides/embed/ui-elements/installation)を参照してください。
...
App Userの`external_app_user_id`は、[`PUT /users/:id`](/reference/put-users-id)エンドポイントを介して更新できます。
...
アプリケーションは、APIコールに対してアクティブ化されたら、会社での再認証が必要になります。この操作を実行する手順については、[こちら](/guides/authorization/platform-app-approval)を参照してください。
...
* 有効な[サービスアカウント](/platform/user-types/#service-account)アクセストークン
* 統合を割り当てるApp UserのID
* ユーザーに割り当てるアプリ統合のID
