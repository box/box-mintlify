...
Boxでタスクを更新するには、タスクのIDを指定して<Link href="/reference/put-tasks-id">`PUT /tasks/:task_id`</Link> APIを呼び出す必要があります。このAPIを使用すると、タスクの`action`タイプの変更、`message`の追加、期日の変更を行うことができます。
