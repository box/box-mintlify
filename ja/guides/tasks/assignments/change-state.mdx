...
タスク割り当ての状態を更新するには、<Link href="/reference/put-task-assignments-id">`PUT /tasks/:task_id/assignments`</Link> APIを呼び出し、`completed`、`incomplete`、`approved`、`rejected`などの`resolution_state`を含めます。
