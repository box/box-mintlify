...
* すべてのコンテンツをあるユーザーから別のユーザーに直接移動する、[所有フォルダの移動](/ja/reference/put-users-id-folders-0)APIを使用する。
...
[所有フォルダの移動エンドポイント](/ja/reference/put-users-id-folders-0)は、あるユーザーが所有するコンテンツ全体を別のユーザーに移動することを目的に設計されています。
...
コラボレーション転送は、[コラボレーションエンドポイント](/ja/reference/post-collaborations)を使用して、単一のファイルまたはフォルダの所有権をあるユーザーから別のユーザーに即座に変更するプロセスです。
...
`transfer_from_user`アカウントとして呼び出しを行い、[コラボレーションを追加](/ja/reference/post-collaborations)エンドポイントを使用して`transfer_to_user`を共同所有者として追加します。
...
`transfer_to_user`アカウントとして呼び出しを実行し、[コラボレーションを取得エンドポイント](/ja/reference/get-collaborations-id)を使用して、転送するファイルまたはフォルダのIDのコラボレーションを取得します。コラボレーションIDをキャプチャします。
...
最後の手順は、ファイルまたはフォルダの所有者として`transfer_from_user`アカウントを削除することです。これは、[コラボレーションを削除エンドポイント](/ja/reference/delete-collaborations-id)を使用して行います。
...
<RelatedLinks
  title="関連するAPI"
  items={[
    { label: "Transfer owned folders", href: "/ja/reference/put-users-id-folders-0", badge: "PUT" }
  ]}
/>
<RelatedLinks
  title="関連するガイド"
