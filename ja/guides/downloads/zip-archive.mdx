...
rank: 11
related_endpoints:
  0: get_zip_downloads_id_content
  1: get_zip_downloads_id_status
  2: post_zip_downloads
related_guides: {}
required_guides: {}
...
ダウンロードに時間がかかる場合は、[ステータスのエンドポイント](/reference/get-zip-downloads-id-status)を使用してダウンロードのステータスを監視できます。これにより、ダウンロードの進行状況のほか、スキップされた可能性のある項目の数を確認できます。
...
<Note>
  SDKを使用してフォルダのコンテンツをダウンロードする場合は、[こちら](/guides/downloads/folder)のガイドに従ってください。
</Note>
