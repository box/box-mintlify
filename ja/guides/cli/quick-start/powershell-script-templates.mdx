---
alias_paths:
  0: /guides/tooling/cli/quick-start/powershell-script-templates
...
スクリプトを使用するには、Box CLIをインストールし、構成する必要があります。まだの場合は、このクイックスタートガイドの<Link href="/guides/cli/quick-start/create-oauth-app">手順1</Link>を参照してください。または、[開発者コンソール][console]に移動し、<Link href="/guides/authentication/oauth2/oauth2-setup">OAuth 2.0を使用した設定</Link>ガイドに従ってください。
...
<Frame border center>
  ![完成したフォルダ構造](/images/guides/cli/final-folder-structure.png)
</Frame>
...
その他のユースケースについては、他の<Link href="/guides/cli/scripts">サンプルスクリプト</Link>を確認してください。
