...
APIレスポンスで返されるデフォルトのフィールドセットです。Standardバリアントは、リソースに対して使用できるメインのAPIを介してそのリソースがリクエストされたときに返されます。たとえば、[`GET /files/:id`](/ja/reference/get-files-id)エンドポイントをリクエストすると、APIはファイルの標準バリエーションを返します。
...
たとえば、[`GET /folders/:id/items`](/ja/reference/get-folders-id-items)エンドポイントをリクエストすると、APIは`item_collection`内でネストされたファイルとフォルダの簡易バリエーションを返します。
...
たとえば、`fields=is_package,lock`パラメータを指定して[`GET /files/:id`](/ja/reference/get-files-id)エンドポイントをリクエストすると、APIは、指定されたフィールドに加えて、そのファイルの基本的なフィールドを返します。
...
<RelatedLinks
  ...
  items={[
    ...
    { label: "Get user", href: "/ja/reference/get-users-id", badge: "GET" }
  ]}
/>
