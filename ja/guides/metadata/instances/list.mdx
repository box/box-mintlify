...
ファイルのすべてのメタデータインスタンスのリストを取得するには、<Link href="/reference/get-files-id-metadata">`GET /files/:file_id/metadata`</Link> APIエンドポイントを呼び出します。
...
任意のフォルダ (ルートフォルダを除く) のすべてのメタデータインスタンスのリストを取得するには、<Link href="/reference/get-files-id-metadata">`GET /folders/:file_id/metadata`</Link> APIエンドポイントを呼び出します。
