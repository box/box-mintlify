...
LLMゲートウェイで使用される<Link href="/guides/box-ai/ai-agents/get-agent-default-config">デフォルトのエージェント構成</Link>の値は、可能な限り高品質の回答を得られるように変更されることがよくあります。

使用中の構成への悪影響を避けるために、以下に示すAIエージェントの構成の履歴を使用して<Link href="/guides/box-ai/ai-agents/ai-agent-overrides">デフォルト構成を上書きする</Link>ことができます。
