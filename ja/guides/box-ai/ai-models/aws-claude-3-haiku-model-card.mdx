...
The **AWS Claude 3 Haiku** model is tailored for various language tasks, including creative writing and conversational AI.
...
| APIモデル名       | `aws__claude_3_haiku`         | <Link href="/guides/box-ai/ai-agents/ai-agent-overrides">Box AI APIでのモデルの上書き</Link>に使用されるモデルの名前。APIを動作させるには、ユーザーがこの名前を正確に指定する必要があります。 |
