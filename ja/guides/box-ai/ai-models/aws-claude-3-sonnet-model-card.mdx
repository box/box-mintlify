...
The **AWS Claude 3 Sonnet** model is designed for advanced language tasks, focusing on comprehension and context handling.
...
| APIモデル名       | `aws__claude_3_sonnet`        | <Link href="/guides/box-ai/ai-agents/ai-agent-overrides">Box AI APIでのモデルの上書き</Link>に使用されるモデルの名前。APIを動作させるには、ユーザーがこの名前を正確に指定する必要があります。 |
