...
After you choose the **Platform App**, you need to <Link href="/guides/authentication/select">select an authentication
method</Link> that your app will use. In this guide we use the
<Link href="/guides/authentication/oauth2">**User Authentication (OAuth 2.0)**</Link> option, as most of our partners
use this type for their integrations.
...
- **Contact Email** - this is set to the developer of the application by default. Keep in mind that once you publish your app, this email is publicly visible to Box users who view your app in the <Link href="/guides/applications/integrations">Integrations</Link>. We recommend to change it to a support email address, so that users can reach out to support in case of any issues with the integration;
...
Use the <Link href="/guides/authentication/tokens/developer-tokens/#create-developer-token">developer token</Link> authenticated to your current developer account.
...
Start with the <Link href="/guides/tooling/postman">Postman collection</Link> to check
specific calls and see what responses they return.
To see example API calls, browse our <Link href="/reference">API reference documentation</Link>.
