...
The values in the <Link href="/guides/box-ai/ai-agents/get-agent-default-config">default agent configuration</Link> used by the LLM gateway often change to achieve the best possible answer quality. 

To make sure your configurations are not affected in a negative way, you can use the historical AI agent configuration provided below to <Link href="/guides/box-ai/ai-agents/ai-agent-overrides">override the default one</Link>.
