...
a call to the <Link href="/reference/get-events">`GET /events`</Link> API with the `stream_type` set to
...
| `SHIELD_ALERT`                                 | Shield detected an anomalous  download, session, location, or malicious content based on enterprise Shield rules. See <Link href="/guides/events/event-triggers/shield-alert-events">shield alert events</Link> for more information. |
