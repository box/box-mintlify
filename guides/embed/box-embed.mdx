...
<Note>
You can also embed only the AI chat interface from a hub. Users can ask questions and get AI-powered answers based on the hub's files, without accessing navigation or file browsing features. For more information, see the <Link href="/guides/embed/box-embed/#box-hubs-ai-chat-embedding">Box Hubs AI Chat embedding</Link> section.
</Note>
...
<Link href="/reference/put-files-id--add-shared-link">`PUT /files/:file_id`</Link> or <Link href="/reference/put-folders-id--add-shared-link">`PUT /files/:file_id`</Link>.

Then you can find this shared link value using the
<Link href="/reference/get-files-id">`GET /files/:id`</Link> or <Link href="/reference/get-folders-id">`GET /folders/:id`</Link> endpoint and passing in the
...
5. Select chat mode:
    ...
    * <Link href="/guides/embed/box-embed/#chat-button">Chat button</Link>
    * <Link href="/guides/embed/box-embed/#chat-widget">Chat widget</Link>
...
| `showCloseButton` | Whether to show the <Link href="/guides/embed/box-embed/#using-the-close-button">X (close) button</Link> in the chat interface. When set to `true`, the close button is displayed. When a user clicks this button, Box generates an event that is sent to the parent web application, enabling you to implement custom closing logic based on the user interaction. |
...
For files, another option is to call the <Link href="/reference/get-files-id">`GET /files/:id`</Link>
