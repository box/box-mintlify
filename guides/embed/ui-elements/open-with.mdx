...
Currently, the element only supports <Link href="/platform/user-types">App Users</Link>
for authentication.
...
- The application's domain must be allowed by Box Tools. Instructions can be found <Link href="/guides/embed/ui-elements/custom-domains">here</Link>. The ideal workflow is to bundle these steps within an installer that also installs Box Tools.
...
<Link href="/reference/put-users-id">`PUT /users/:id`</Link> endpoint.
...
![Enable Integrations](/images/guides/embed/ui-elements/enable-integrations.png)
...
available <Link href="/guides/authorization/platform-app-approval">here</Link>.
...
- A valid <Link href="/platform/user-types/#service-account">Service Account</Link> Access Token.
...
<Link href="/platform/user-types/#service-account">Service Account</Link> Access Token.
...
More information on scopes can be found <Link href="/guides/api-calls/permissions-and-errors/scopes">here</Link>.
