...
the [`GET /files/:id`](/reference/get-files-id) endpoint the API will return
...
[`GET /folders/:id/items`](/reference/get-folders-id-items) endpoint the API
...
For example, when requesting the [`GET /files/:id`](/reference/get-files-id)
