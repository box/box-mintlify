...
See the <Link href="/reference/resources/client-error">Client Error resource</Link> for more details.
...
| **Solution** | Your application tried to access the Box API from a website. The application needs to <Link href="/guides/security/cors/#enabling-cors-for-your-domain">explicitly allow Cross Origin Resource Sharing</Link> for the domain your site is hosted on.    |
...
| **Solution** | The client is performing operations too quickly and has been rate limited. Client is advised to retry their request after the amount of time specified by the `retry-after` header. There are <Link href="/guides/api-calls/permissions-and-errors/rate-limits">four rate limits</Link> to be aware of. |
